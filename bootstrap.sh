...
# 3. Avoid using service command and just use the usual startup command 
...
su -s /bin/bash hdfs -c "cd /var/lib/hadoop-hdfs && /usr/lib/hadoop/sbin/hadoop-daemon.sh --config '$HADOOP_CONF_DIR' start namenode"
